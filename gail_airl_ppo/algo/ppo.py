...
class PPO(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device, seed, gamma=0.995,
                 ...
                 max_grad_norm=10.0):
        super().__init__(state_shape_multi, action_shape, device, seed, gamma)

        # Rollout buffer.
        self.buffer = RolloutBuffer(
            buffer_size=rollout_length,
            state_shape_multi=state_shape,
...
class PPO_Multi(Algorithm):

    def __init__(self, state_shape_1, state_shape_2, state_shape_multi, action_shape_1, action_shape_2, action_shape_multi, device, seed, gamma=0.995,
                 rollout_length=2048, mix_buffer=20, lr_actor=3e-4,
                 lr_critic=3e-4, units_actor=(64, 64), units_critic=(64, 64),
                 epoch_ppo=10, clip_eps=0.2, lambd=0.97, coef_ent=0.0,
                 max_grad_norm=10.0):
        super().__init__(state_shape_multi, action_shape_multi, device, seed, gamma)

        # Rollout buffer.
        self.buffer_1 = RolloutBuffer(
            buffer_size=rollout_length,
            state_shape=state_shape_1,
            action_shape=action_shape_1,
            device=device,
            mix=mix_buffer
        )

        self.buffer_2 = RolloutBuffer(
            buffer_size=rollout_length,
            state_shape=state_shape_2,
            action_shape=action_shape_2,
            device=device,
            mix=mix_buffer
        )
        ...
        self.actor = StateIndependentPolicy(
            ...
            action_shape=action_shape_multi,
            hidden_units=units_actor,
            hidden_activation=nn.Tanh()
        ).to(device)
    ...
    def step(self, env_1, state_1, env_2, state_2, t, step):
        t += 1
        state_multi = np.concatenate([state_1, state_2], axis=-1)
        action_multi, log_pi = self.explore(state_multi)
        action_1 = action_multi[:3]
        action_2 = action_multi[3:]
        next_state_1, reward_1, done_1, _ = env_1.step(action_1)
        next_state_2, reward_2, done_2, _ = env_2.step(action_2)
        next_state_multi = np.concatenate([next_state_1, next_state_2], axis=-1)
        mask_1 = False if t == env_1._max_episode_steps else done_1
        mask_2 = False if t == env_2._max_episode_steps else done_2
        
        # Averaging the rewards
        self.buffer_1.append(state_1, action_1, (reward_1 + reward_2)/2, mask_1, log_pi, next_state_1)
        self.buffer_2.append(state_2, action_2, (reward_1 + reward_2)/2, mask_1, log_pi, next_state_2)

        if done_1:
            next_state_1 = env_1.reset()
        if done_2:
            next_state_2 = env_2.reset()

        if done_1 and done_2:
            t = 0

        return next_state_1, next_state_2, t
